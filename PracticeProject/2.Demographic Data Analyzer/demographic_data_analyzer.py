...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = ((df["hours-per-week"] == min_work_hours) & (
                df["salary"] == '>50K')).sum() / num_min_workers * 100

    # What country has the highest percentage of people that earn >50K?
    df['high_salary'] = (df['salary'] == '>50K')
    group = df.groupby('native-country')['high_salary'].mean().sort_values(ascending=False)
    highest_earning_country = group.index[0]
    highest_earning_country_percentage = round(group.values[0]*100, 1)
    del df['high_salary']
    del group
